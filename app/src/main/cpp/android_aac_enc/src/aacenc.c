...
/*******************************************************************************
	...
	Content:	aac h264Encoder interface functions
...
VO_U32 VO_API voAACEncInit(VO_HANDLE * phCodec,VO_AUDIO_CODINGTYPE vType, VO_CODEC_INIT_USERDATA *pUserData)
{
	...
	/* init the aac h264Encoder handle */
	...
	if(!error)
	{
		/* init the aac h264Encoder intra memory */
	...
	if (!error) {
		/* init the aac h264Encoder psychoacoustic */
		error = (PsyNew(&hAacEnc->psyKernel, MAX_CHANNELS, pMemOP) ||
			PsyOutNew(&hAacEnc->psyOut, pMemOP));
	}

	if (!error) {
		/* init the aac h264Encoder quantization elements */
		error = QCOutNew(&hAacEnc->qcOut,MAX_CHANNELS, pMemOP);
	}

	if (!error) {
		/* init the aac h264Encoder quantization state */
		error = QCNew(&hAacEnc->qcKernel, pMemOP);
	}

	/* uninit the aac h264Encoder if error is nozero */
	if(error)
	{
	...
	/* init the aac h264Encoder memory operator  */
#ifdef USE_DEAULT_MEM
	if(interMem)
	{
...
#endif
	/* init the aac h264Encoder default parameter  */
	if(hAacEnc->initOK == 0)
	{
		 AACENC_CONFIG config;
		 config.adtsUsed = 1;
...
VO_U32 VO_API voAACEncUninit(VO_HANDLE hCodec)
{
	...
	if(NULL != hAacEnc)
	{
		/* close the aac h264Encoder */
		AacEncClose(hAacEnc, hAacEnc->voMemop);

		/* free the aac h264Encoder handle*/
...
VO_U32 VO_API voAACEncSetParam(VO_HANDLE hCodec, VO_S32 uParamID, VO_PTR pData)
{
	...
	switch(uParamID)
	{
	case VO_PID_AAC_ENCPARAM:  /* init aac h264Encoder parameter*/
