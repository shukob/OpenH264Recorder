    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        when (item?.itemId) {
            R.id.action_stop -> {
                ...
                progressDialogHelper.updateProgressDialogMessage(this@MainActivity, "動画を生成中です")
                mp4Recorder.stop {
                    progressDialogHelper.hideProgressDialog(this@MainActivity)
                    ErrorDialogHelper().showMessageDialogWithMessage(this@MainActivity, "動画を生成しました",
                            "生成完了", "再生", object : Runnable {
                        override fun run() {
                            val intent = Intent(Intent.ACTION_VIEW, Uri.fromFile(mp4Recorder.outputFile))
